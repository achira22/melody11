...
module.exports = {
  ...
  run: async (client, message,args) => {
    ...
    if (!args[0]) return functions.embedbuilder(client, 5000, message, config.colors.no, "`" + message.author.tag + "`" + " Please add something you wanna search to").then((msg) => {
      msg.delete(({
        timeout: 5000
      }))
    ...
    if (args.join(" ").toLowerCase().includes('open.spotify') && args1.join(" ").toLowerCase().includes('track')) {
      //get data
    ...
    // code of playlist
    else if (args.join(" ").toLowerCase().includes('open.spotify') && args1.join(" ").toLowerCase().includes('playlist')) {
    ...
    message.channel.send(
      new MessageEmbed()
        .setColor(ee.colour)
        .setTitle(`Searching Song`)
        .setDescription(`🎶  I AM Searching Your Song \`\`\`${args.join(" ")}\`\`\``)
        .setFooter(config.footertext)
    ).then((msg) => {
      msg.delete(({
        timeout: 5000
